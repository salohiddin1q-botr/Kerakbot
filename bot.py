...
def generate_admin_code() -> str:
    """20 daqiqa amal qiladigan bir martalik kod yaratadi."""
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO admin_codes (code, expires_at, used) VALUES (?, ?, 0)",
        (code, expires_at),
    )
    conn.commit()
    conn.close()
...
def generate_user_login_code(owner_user_id: int) -> str:
    """10 daqiqa amal qiladigan, foydalanuvchining o'z hisobiga
    boshqa qurilmadan kirish uchun bir martalik kod yaratadi."""
    code = str(random.randint(100000, 999999))
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO user_login_codes (code, owner_user_id, expires_at, used) VALUES (?, ?, ?, 0)",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/search <so'z> — xabarlar orasidan qidirish\n"
        "/mycode — boshqa telefondan hisobingizga kirish uchun kod olish\n"
        "/help — yordam"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/mycode — shu yerda kod olasiz\n"
        "/login <kod> — boshqa telefonda shu kodni kiritasiz"
    )
...
async def mycode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Har qanday foydalanuvchi — o'z hisobiga boshqa telefondan
    kirish uchun vaqtinchalik kod yaratadi."""
    user = update.effective_user
    owner_id = get_effective_user_id(user.id)  # agar allaqachon sessiyada bo'lsa, asl egaga tegishli kod

    code = generate_user_login_code(owner_id)
    await update.message.reply_text(
        f"🔑 Sizning shaxsiy kodingiz: {code}\n\n"
        f"Boshqa telefonda botga o'ting va yozing:\n/login {code}\n\n"
        f"Bu kod {USER_LOGIN_MINUTES} daqiqa amal qiladi, faqat bir marta ishlatiladi.\n"
        f"⚠️ Bu kodni hech kimga bermang — kim kiritsa, sizning xabarlaringizni ko'ra oladi."
    )
...
async def gencode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Faqat asosiy admin (ADMIN_ID) uchun — vaqtinchalik kod yaratadi."""
    user = update.effective_user
    ...
    await update.message.reply_text(
        f"🔑 Vaqtinchalik admin kodi: {code}\n\n"
        f"Boshqa telefonda botga yozing:\n/unlock {code}\n\n"
        f"Bu kod {TEMP_ADMIN_MINUTES} daqiqa amal qiladi va faqat bir marta ishlatiladi."
    )

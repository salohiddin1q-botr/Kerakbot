...
def get_message_by_id_admin(target_user_id: int, message_id: int):
    """Admin uchun — istalgan foydalanuvchining bitta xabarini to'liq ma'lumoti bilan qaytaradi."""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        """SELECT content_type, text, file_id, saved_at FROM messages
...
async def viewuser_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Yoki bitta xabarni to'liq ko'rish uchun:\n/viewuser 847362915 5"
        )
        return
    ...
    # Agar xabar ID ham berilgan bo'lsa — aynan o'sha xabarni to'liq (rasm/video/fayl bilan) ko'rsatish
    if len(context.args) >= 2:
        try:
        ...
        row = get_message_by_id_admin(target_id, message_id)
        if not row:
        ...
        if content_type == "text":
            await update.message.reply_text(f"👤 {target_id} | 📝 {saved_at}\n\n{text}")
        elif content_type == "photo":
            await update.message.reply_photo(photo=file_id, caption=f"👤 {target_id}\n{text or ''}")
        elif content_type == "document":
            await update.message.reply_document(document=file_id, caption=f"👤 {target_id}\n{text or ''}")
        elif content_type == "video":
            await update.message.reply_video(video=file_id, caption=f"👤 {target_id}\n{text or ''}")
        return
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        "/viewuser <user_id> <xabar_id> — bitta xabarni to'liq (rasm/video bilan) ko'rish",
